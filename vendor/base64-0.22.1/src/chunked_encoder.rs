...
pub trait Sink {
    ...
    fn write_encoded_bytes(&mut self, encoded: &str) -> Result<(), Self::Error>;
}
...
impl<'e, E: Engine + ?Sized> ChunkedEncoder<'e, E> {
    ...
    pub fn encode<S: Sink>(&self, bytes: &str, sink: &mut S) -> Result<(), S::Error> {
...
impl<'a> Sink for StringSink<'a> {
    ...
    fn write_encoded_bytes(&mut self, s: &str) -> Result<(), Self::Error> {
...
pub mod tests {
    ...
    fn chunked_encode_str(bytes: &str, config: GeneralPurposeConfig) -> String {
    ...
    pub trait SinkTestHelper {
        fn encode_to_string<E: Engine>(&self, engine: &E, bytes: &str) -> String;
    }
