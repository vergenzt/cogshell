...
/// A wrapper for `&str` that provides convenient string oriented trait impls.
///
/// If you need ownership or a growable byte string buffer, then use
/// [`BString`](struct.BString.html).
///
/// Using a `&BStr` is just like using a `&str`, since `BStr`
...
impl BStr {
    ...
    /// returns a `&BStr` instead of a `&str`.
    ///
    ...
    #[inline]
    pub(crate) fn from_bytes(slice: &str) -> &BStr {
